...
- 主工作區窗口由留言 `【OWNER想法】` 後的 `【全員回覆：24小時】` 開啟；期限固定為連續 24 小時，從通知留言建立時間起算，開啟後不可延長、重開或使用其他期限。舊版 `【全員回覆：N天】` 格式已停用，用來開新窗口會直接回錯誤，不會建立窗口。
...
在主協作 workspace，留言仍使用同一個 endpoint。OWNER 先以六欄格式留下 `【OWNER想法】`，再以 `【全員回覆：24小時】` 通知 user02-06 與 user09；合法通知會在同一交易內與 `main_discussion_windows` 保存固定的 `opened_at`、`wait_half_days`（固定為 `2`）、`due_at`（開啟時間 +24 小時）。留言失敗時窗口與 mention notification 都不會留下。其他討論與一般 workspace 留言維持原本 CRUD 行為。
