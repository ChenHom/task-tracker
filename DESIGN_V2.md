# Design v2 需求總覽（已實作基線）

> 接續 [DESIGN.md](DESIGN.md)。本文保留 v2 的需求與設計決策；截至 2026-07-10，Seeder、忘記密碼、Member API 與前端 SPA 均已實作，驗證與後續項目以 [TASKS_V2.md](TASKS_V2.md) 為準。

## 原始缺口（均已完成）

1. **建立使用者** — 已完成 `createUser` 與 idempotent seeder。
2. **Member 邀請 API** — 已完成邀請、接受、角色變更與移除路由。
3. **前端串接** — 已完成無 framework 的 Native ESM SPA。
4. **忘記密碼（Email 重設連結）** — 已完成 token 生命週期、假寄信與 session 失效。

---

## 1. 建立使用者 + Seeder

**實作狀態（2026-07-10）**：已完成，見 [TASKS_V2.md](TASKS_V2.md#phase-8--建立使用者--seeder-)。
...
**原始背景（實作前）**：Member 邀請、前端登入都需要真實帳號才能端到端測；seeder 直接寫 30 筆到 db，比做完整註冊流程更快讓其他兩塊可測。

**原始驗收範圍**：
- `createUser(email, password)` 內部函式（複用既有 `hashPassword`，寫進 `users` table）
- 重複 email 要回乾淨的錯誤（當時 UNIQUE constraint 會直接丟 SQLite 例外，需包裝）
...
**實作狀態（2026-07-10）**：已完成，包含 invited/joined 權限邊界與 Owner 守門，見 [TASKS_V2.md](TASKS_V2.md#phase-10--member-邀請-api-rbac-)。

**原始背景（實作前）**：當時 domain 邏輯已測過，但尚缺 HTTP 路由。`member.invited` 與 `member.joined` 的獨立事件也促成了現行「邀請後無權限，接受後才生效」的兩步驟設計。
...
- invited 但未 joined 沒有任何存取權限；`member.ts` 的 projection 維持「invited 不投影，joined 才進 read model」，HTTP join route 也已實作
...
**已完成的實作點**：`archiveWorkspace` / `deleteWorkspace`（[workspace.ts](src/workspace.ts)）已加上「`workspace_members_read_model` 的 active 成員數 == 1」守門。HTTP 路由與前端入口仍列在 Phase 13。
...
**實作狀態（2026-07-10）**：已完成基礎 SPA 與後續模組化，見 [TASKS_V2.md](TASKS_V2.md#phase-11--前端串接-)。

**原始背景（實作前）**：當時後端已有 40+ 條路由，但還沒有前端消費，系統只能透過 curl 操作。

**原始驗收範圍**（粗略頁面清單）：登入、workspace 列表與切換、task 列表（含狀態機合法轉換的 UI 限制）、comment、attachment 上傳/下載、search、member 管理（依賴 #2）、audit 檢視
...
**當期明確不做**：CSS 排版打磨、拖拉看板、即時更新、任何前端 build 工具鏈
...
**實作狀態（2026-07-10）**：已完成，見 [TASKS_V2.md](TASKS_V2.md#phase-9--忘記密碼email-重設連結)。
...
- `POST /api/auth/reset-password`（token + 新密碼）→ 驗證 token 存在、未過期、未使用過，通過後更新密碼、把 token 標記已使用、**並讓該使用者其他裝置的 session 全部失效**（當時 `sessions` table 還沒有依 `user_id` 批次刪除操作）
...
2026-07-10 的 AI session 巡檢已將跨 workspace 搬移 task、workspace 封存入口與台北時區顯示收斂到 [TASKS_V2.md](TASKS_V2.md#phase-13--ai-session-巡檢交接2026-07-10)。
