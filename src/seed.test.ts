...
seedUsers(db2);
const legacyRows = db2.prepare("SELECT email, name FROM users WHERE email IN ('user02@test.local', 'user03@test.local', 'user04@test.local') ORDER BY email").all() as
  { email: string; name: string }[];
assert.deepStrictEqual(legacyRows.map((user) => ({ email: user.email, name: user.name })), [
  { email: 'user02@test.local', name: '小美' },
  { email: 'user03@test.local', name: '阿凱' },
  { email: 'user04@test.local', name: '既有姓名' },
]);
